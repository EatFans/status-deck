...
每次成功解析后，桌面端会将最近一次可信快照缓存到系统缓存目录。即使 Codex 额度
耗尽后不再产生新的本地会话事件，缓存中的 `0%` 仍会持续发送到设备端，直至对应
窗口的重置时间。若尚未使用过 Codex、会话记录中没有额度事件，或缓存已经跨过额度
重置时间而尚未等到新事件，桌面端才会发送 `available: false`，设备端会清除过期额度
并显示数据暂不可用。本地事件的字段仍可能随 Codex 更新变化，但不会影响账户安全或
产生网络请求。
